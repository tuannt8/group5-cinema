...
num_iters = 10;
disp(['Performing reconstrution using cgls running for ' num2str(num_iters) ' iterations'])
tic
...
%% Run for another 50 iterations
num_iters2 = 10;
disp(['A continous run for further ' num2str(num_iters2) ' iterations'])
tic
astra_mex_algorithm('iterate', alg_id, num_iters2);
run_time2 = toc();
disp(['Took ' num2str(run_time2) ' for the rerun.'])

%% Retrieve our reconstruction again and display it.
reconstruction = astra_mex_data3d('get', rec_id);
figure
for i = 1:size(reconstruction,3)
    show_image(rot90(squeeze(reconstruction(:,:,i))));
